<div class="card mb-3">
    <div class="card-body">
        <div class="row justify-content-between align-items-center">
            <div class="col-md">
                <h5 class="mb-2 mb-md-0">Reservation #{{ $reservation->no_request_reservation }}</h5>
            </div>
    ...
    <div class="card-body">
        ...
        <div class="row align-items-center">
            <div class="col">
                ...
                <h5>{{ $reservation->Ticket->Tenant->nama_tenant }}</h5>
                <p class="fs--1">
                    {{ Auth::user()->Site->nama_site }},
                    {{ $reservation->Ticket->Unit->Tower->nama_tower }}
                    {{ $reservation->Ticket->Unit->nama_unit }}<br />
                    {{ Auth::user()->Site->provinsi }}, {{ Auth::user()->Site->kode_pos }}
                </p>
                <p class="fs--1"><a
                        href="mailto:{{ $reservation->Ticket->Tenant->email_tenant }}">{{ $reservation->Ticket->Tenant->email_tenant }}</a><br /><a
                        href="tel:444466667777">{{ $reservation->Ticket->Tenant->no_telp_tenant }}</a></p>
            </div>
                    ...
                    <table class="table table-sm table-borderless fs--1">
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    {{ HumanDate($reservation->CashReceipt->created_at) }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <table class="table border-bottom">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="align-middle">
                            <p class="mb-0">
                                {{ $reservation->RuangReservation->ruang_reservation }}
                            </p>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="align-middle">
                            <p class="mb-0">
                                {{ $reservation->JenisAcara->jenis_acara }}
                            </p>
                        </td>
                    </tr>
                    <tr>
        ...
        @if ($reservation->CashReceipt->transaction_status == 'PENDING')
            <form action="{{ route('generatePaymentRSV', $reservation->id) }}" method="post">
                @csrf
                        ...
                        <div class="card h-100">
                            ...
                            <div class="card-body bg-light">
                                <div class="d-flex justify-content-between fs--1 mb-1">
                                    ...
                                    <span>{{ rupiah($reservation->CashReceipt->sub_total) }}</span>
                                </div>
        ...
        @elseif ($reservation->CashReceipt->transaction_status == 'VERIFYING')
            <div class="text-center">
                <a href="{{ route('paymentReservation', [$reservation->id, $reservation->CashReceipt->id]) }}"
...
<script>
    ...
    var admin_tax = 0.11;
    var subtotal = parseInt('{{ $reservation->CashReceipt->sub_total }}')
    var isCCForm = false;
    ...
    var token = '{{ $reservation->CashReceipt->snap_token }}'
    var periode_bulan = "{{ $reservation->CashReceipt->periode_bulan }}"
    var periode_tahun = '{{ $reservation->CashReceipt->periode_tahun }}'

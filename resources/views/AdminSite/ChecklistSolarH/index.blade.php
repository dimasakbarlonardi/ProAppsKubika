                        ...
                        <div class="d-lg-flex justify-content-between">
                            <div class="row flex-between-center gy-2 px-x1 text-light">
                                <div class="col-auto pe-0">
                                    <h6 class="mb-0 text-light">List Check list Solar</h6>
                                </div>
                            </div>
                            ...
                            <div class="d-flex align-items-center justify-content-between justify-content-lg-end px-x1">
                                ...
                                <div class="col-auto d-flex">
                                    <a class="btn btn-falcon-default btn-sm text-600" href="{{ route('checklistsolars.create') }}"><span class="fas fa-plus fs--2 me-1"></span>Create Check list Solar</a>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card-body">
                            <form>
                                {{-- <div class="mb-3 mt-n2">
                                    <label class="mb-1">Tanggal Checklist Solar</label>
                                    <select class="form-select form-select-sm" name="tgl_checklist" required id="tgl_checklist">
                                        @foreach ($checklistsolars as $checklistsolar)
                                            <option value="{{ $checklistsolar->tgl_checklist }}"> {{ $checklistsolar->tgl_checklist }}</option>
                                        @endforeach
                                    </select>
                                </div> --}}
                                <div class="mb-3 mt-n2">
                                    <label class="form-label" for="timepicker2">Tanggal Checklist Solar</label>
                                ...
                                <div class="mb-3 mt-n2">
                                    <label class="mb-1">Nomer Checklist Solar</label>
...
@section('script')
    ...
    <script>
        $('document').ready(function() {
            var no_checklist_solar = $('#no_checklist_solar').val()
            ...
            $('#no_checklist_solar').on('change', function() {
                var no_checklist_solar = $('#no_checklist_solar').val()
                var tgl_checklist = $('#tgl_checklist').val()
                var date_from = tgl_checklist.substr(0, 10)
                var date_to = tgl_checklist.substr(14, 23)
        ...
        function index(no_checklist_solar, date_from, date_to) {
            $.ajax({
                url: '/admin/checklist-filter-solar',
                type: 'GET',
                data: {
                    no_checklist_solar,
                    date_from,

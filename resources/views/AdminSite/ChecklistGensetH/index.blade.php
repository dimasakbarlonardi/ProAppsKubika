                        ...
                        <div class="d-lg-flex justify-content-between">
                            <div class="row flex-between-center gy-2 px-x1 text-light">
                                <div class="col-auto pe-0">
                                    <h6 class="mb-0 text-light">List Check list Genset</h6>
                                </div>
                            </div>
                            ...
                            <div class="d-flex align-items-center justify-content-between justify-content-lg-end px-x1">
                                ...
                                <div class="col-auto d-flex">
                                    <a class="btn btn-falcon-default btn-sm text-600" href="{{ route('checklistgensets.create') }}"><span class="fas fa-plus fs--2 me-1"></span>Create Check list Genset</a>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="sort" data-sort="no_checklist_genset">Nomer Check list Genset</th>
                                    <th class="sort">Action</th>
                                </tr>
                            </thead>
                            <tbody id="checklist_body">
                                @foreach ($checklistgensets as $key => $checklistgenset)
                                    <tr>
                                        <th scope="row">{{$key + 1 }}</th>                           
                                        <td>{{ $checklistgenset->tgl_checklist }}</td>                            
                                        <td>{{ $checklistgenset->no_checklist_genset }}</td>
                                        <td>
                                            <div class="dropdown font-sans-serif position-static"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal" type="button" id="order-dropdown-0" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                                <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="order-dropdown-0">
                                                    <div class="py-2"><a class="dropdown-item text" href="{{ route('checklistgensets.show', $checklistgenset->no_checklist_genset) }}">Detail Genset Checklist</a>
                                                </div>
                                            </div>  
                                         </div>                      
                                        </td>
...
@section('script')
    ...
    <script>
        $('document').ready(function() {
            var no_checklist_genset = $('#no_checklist_genset').val()
            ...
            $('#tgl_checklist').on('change', function() {
                ...
                index(no_checklist_genset, date_from, date_to)
            })
            $('#no_checklist_genset').on('change', function() {
                var no_checklist_genset = $('#no_checklist_genset').val()
                var tgl_checklist = $('#tgl_checklist').val()
                var date_from = tgl_checklist.substr(0, 10)
                var date_to = tgl_checklist.substr(14, 23)

                // console.log(tgl_checklist, no_checklist_genset)

                index(no_checklist_genset, date_from, date_to)
            })
        })

        function index(no_checklist_genset, date_from, date_to) {
            $.ajax({
                url: '/admin/checklist-filter-genset',
                type: 'GET',
                data: {
                    no_checklist_genset,
                    date_from,
                    ...
                    data.checklists.map((item, i) => {
                        $('#checklist_body').append(`
                            <tr>
                                ...
                                <td>${item.no_checklist_genset}</td>
                                <td>
                                    <div class="dropdown font-sans-serif position-static"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal" type="button" id="order-dropdown-0" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                        <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="order-dropdown-0">
                                            <a class="dropdown-item text" href="/admin/checklistgensets/${item.no_checklist_genset}">Detail genset Checklist</a>
                                        </div>
                                    </div>                        
                                </td>
                            </tr>

    ...
    <div class="card">
        <div class="card-header py-2">
            <div class="row flex-between-center">
                <div class="col-auto">
                    <h6 class="my-3 text-light">Create Checklist Office Management</h6>
                </div>
            </div>
        </div>
        <div class="p-5">
            <form method="post" action="{{ route('checklistoffices.store') }}">
                ...
                <div class="col-6 mb-3">
                    <label class="form-label">HK Office Management</label>
                    <select class="form-control" name="id_hk_office" required>
                        <option selected disabled>-- Pilih HK Office Management --</option>
                        @foreach ($engoffices as $engoffice)
                        <option value="{{ $engoffice->id_hk_office }}">{{ $engoffice->nama_hk_office }} : {{$engoffice->subject}} </option>
                        @endforeach
                    </select>
                </div>
                <div class="col-6 mb-3">
                ...
                <div class="row mt-4">
                    <h6>ISI DETAIL CHECKLIST OFFICE MANAGEMENT <hr></h6>

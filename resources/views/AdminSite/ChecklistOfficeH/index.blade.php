                        ...
                        <div class="d-lg-flex justify-content-between">
                            <div class="row flex-between-center gy-2 px-x1 text-light">
                                <div class="col-auto pe-0">
                                    <h6 class="mb-0 text-light">List Check list Office Management</h6>
                                </div>
                            </div>
                            ...
                            <div class="d-flex align-items-center justify-content-between justify-content-lg-end px-x1">
                                ...
                                <div class="col-auto d-flex">
                                    <a class="btn btn-falcon-default btn-sm text-600" href="{{ route('checklistoffices.create') }}"><span class="fas fa-plus fs--2 me-1"></span>Create Check list Office Management</a>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="sort" data-sort="no_checklist_office_management">Nomer Check list Office Management</th>
                                    <th class="sort">Action</th>
                                </tr>
                            </thead>
                            <tbody id="checklist_body">
                                @foreach ($checklistoffices as $key => $checklistoffice)
                                    <tr>
                                        <th scope="row">{{$key + 1 }}</th>                           
                                        <td>{{ $checklistoffice->tgl_checklist }}</td>                            
                                        <td>{{ $checklistoffice->no_checklist_office_management }}</td>
                                        <td>
                                            <div class="dropdown font-sans-serif position-static"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal" type="button" id="order-dropdown-0" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                                <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="order-dropdown-0">
                                                    <div class="py-2"><a class="dropdown-item text" href="{{ route('checklistoffices.show', $checklistoffice->no_checklist_office_management) }}">Detail Office Management Checklist</a>
                                                </div>
                                            </div>  
                                         </div>                      
                                        </td>
                        ...
                        <div class="card-body">
                            <form>
                                ...
                                <div class="mb-3 mt-n2">
                                    <label class="form-label" for="timepicker2">Tanggal Checklist Office Management</label>
                                ...
                                <div class="mb-3 mt-n2">
                                    <label class="mb-1">Nomer Checklist Office Management</label>
                                    <select class="form-select form-select-sm" name="no_checklist_office_management" required id="no_checklist_office_management">
                                        <option type="reset" value=""> All </option>
                                        @foreach ($checklistoffices as $checklistoffice)
                                            <option value="{{ $checklistoffice->no_checklist_office_management }}"> {{ $checklistoffice->no_checklist_office_management }}</option>
                                        @endforeach
                                    </select>
                                </div>     
                                <div class="mb-3 mt-n2">
                                    <label class="mb-1">User Checklist Office Management</label>
                                    <select class="form-select form-select-sm" name="no_checklist_office_management" required id="no_checklist_office_management">
...
@section('script')
    ...
    <script>
        $('document').ready(function() {
            var no_checklist_office_management = $('#no_checklist_office_management').val()
            ...
            $('#tgl_checklist').on('change', function() {
                ...
                index(no_checklist_office_management, date_from, date_to)
            })
            $('#no_checklist_office_management').on('change', function() {
                var no_checklist_office_management = $('#no_checklist_office_management').val()
                var tgl_checklist = $('#tgl_checklist').val()
                var date_from = tgl_checklist.substr(0, 10)
                var date_to = tgl_checklist.substr(14, 23)

                // console.log(tgl_checklist, no_checklist_office_management)

                index(no_checklist_office_management, date_from, date_to)
            })
        })

        function index(no_checklist_office_management, date_from, date_to) {
            $.ajax({
                url: '/admin/checklist-filter-office_management',
                type: 'GET',
                data: {
                    ...
                    data.checklists.map((item, i) => {
                        $('#checklist_body').append(`
                            <tr>
                                ...
                                <td>${item.no_checklist_office_management}</td>
                                <td>
                                    <div class="dropdown font-sans-serif position-static"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal" type="button" id="order-dropdown-0" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                        <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="order-dropdown-0">
                                            <a class="dropdown-item text" href="/admin/checklistoffices/${item.no_checklist_office_management}">Detail Office Management Checklist</a>
                                        </div>
                                    </div>                        
                                </td>
                            </tr>

    ...
    <div class="card">
        <div class="card-header py-2">
            <div class="row flex-between-center">
                <div class="col-auto">
                    <h6 class="my-3 text-light">Create Checklist Listrik</h6>
                </div>
            </div>
        </div>
        <div class="p-5">
            <form method="post" action="{{ route('checklistlistriks.store') }}">
                ...
                <div class="col-6 mb-3">
                    <label class="form-label">Eng Listrik</label>
                    <select class="form-control" name="id_eng_listrik" required>
                        <option selected disabled>-- Pilih Enggeneering Listrik --</option>
                        @foreach ($englistriks as $englistrik)
                        <option value="{{ $englistrik->id_eng_listrik }}">{{ $englistrik->nama_eng_listrik }} : {{$englistrik->subject}} </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="row mt-4">
                    <h6>ISI DETAIL CHECKLIST LISTRIK <hr></h6>

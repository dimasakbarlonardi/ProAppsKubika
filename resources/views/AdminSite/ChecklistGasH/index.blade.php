                        ...
                        <div class="d-lg-flex justify-content-between">
                            <div class="row flex-between-center gy-2 px-x1 text-light">
                                <div class="col-auto pe-0">
                                    <h6 class="mb-0 text-light">List Check list Gas</h6>
                                </div>
                            </div>
                            ...
                            <div class="d-flex align-items-center justify-content-between justify-content-lg-end px-x1">
                                ...
                                <div class="col-auto d-flex">
                                    <a class="btn btn-falcon-default btn-sm text-600" href="{{ route('checklistgases.create') }}"><span class="fas fa-plus fs--2 me-1"></span>Create Check list Gas</a>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="sort" data-sort="no_checklist_gas">Nomer Check list Gas</th>
                                    <th class="sort">Action</th>
                                </tr>
                            </thead>
                            <tbody id="checklist_body">
                                @foreach ($checklistgases as $key => $checklistgas)
                                    <tr>
                                        <th scope="row">{{$key + 1 }}</th>                           
                                        <td>{{ $checklistgas->tgl_checklist }}</td>                            
                                        <td>{{ $checklistgas->no_checklist_gas }}</td>
                                        <td>
                                            <div class="dropdown font-sans-serif position-static"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal" type="button" id="order-dropdown-0" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                                <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="order-dropdown-0">
                                                    <div class="py-2"><a class="dropdown-item text" href="{{ route('checklistgases.show', $checklistgas->no_checklist_gas) }}">Detail Gas Checklist</a>
                                                </div>
                                            </div>  
                                         </div>                      
                                        </td>
                        ...
                        <div class="card-body">
                            <form>
                                {{-- <div class="mb-3 mt-n2">
                                    <label class="mb-1">Tanggal Checklist gas</label>
                                    <select class="form-select form-select-sm" name="tgl_checklist" required id="tgl_checklist">
                                        @foreach ($checklistgases as $checklistgas)
                                            <option value="{{ $checklistgas->tgl_checklist }}"> {{ $checklistgas->tgl_checklist }}</option>
                                        @endforeach
                                    </select>
                                </div> --}}
                                <div class="mb-3 mt-n2">
                                    <label class="form-label" for="timepicker2">Tanggal Checklist Gas</label>
                                ...
                                <div class="mb-3 mt-n2">
                                    <label class="mb-1">Nomer Checklist Gas</label>
                                    <select class="form-select form-select-sm" name="no_checklist_gas" required id="no_checklist_gas">
                                        @foreach ($checklistgases as $checklistgas)
                                            <option value="{{ $checklistgas->no_checklist_gas }}"> {{ $checklistgas->no_checklist_gas }}</option>
                                        @endforeach
                                    </select>
                                </div>                        
                            </form>
...
@section('script')
    ...
    <script>
        $('document').ready(function() {
            var no_checklist_gas = $('#no_checklist_gas').val()
            ...
            $('#tgl_checklist').on('change', function() {
                ...
                index(no_checklist_gas, date_from, date_to)
            })
            $('#no_checklist_gas').on('change', function() {
                var no_checklist_gas = $('#no_checklist_gas').val()
                var tgl_checklist = $('#tgl_checklist').val()
                var date_from = tgl_checklist.substr(0, 10)
                var date_to = tgl_checklist.substr(14, 23)

                // console.log(tgl_checklist, no_checklist_gas)

                index(no_checklist_gas, date_from, date_to)
            })
        })

        function index(no_checklist_gas, date_from, date_to) {
            $.ajax({
                url: '/admin/checklist-filter-gas',
                type: 'GET',
                data: {
                    no_checklist_gas,
                    date_from,
                    ...
                    data.checklists.map((item, i) => {
                        $('#checklist_body').append(`
                            <tr>
                                ...
                                <td>${item.no_checklist_gas}</td>
                                <td>
                                    <div class="dropdown font-sans-serif position-static"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal" type="button" id="order-dropdown-0" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                        <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="order-dropdown-0">
                                            <a class="dropdown-item text" href="/admin/checklistgases/${item.no_checklist_gas}">Detail gas Checklist</a>
                                        </div>
                                    </div>                        
                                </td>
                            </tr>

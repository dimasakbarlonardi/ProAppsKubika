        ...
        <div class="card-header py-2">
            <div class="row flex-between-center">
                ...
                <div class="col-auto d-flex">
                    <a class="btn btn-falcon-default text-600 btn-sm" style="margin-right: 10px" href="{{ route('toolshousekeeping.create') }}">
                        <span class="fas fa-plus fs--2 me-1"></span>
                        Create Tools HouseKeeping
                    </a>
                    <a class="btn btn-falcon-default text-600 btn-sm" href="{{ route('historyToolHK') }}">
                ...
                <tbody id="checklist_body">
                    @foreach ($toolshk as $key => $tools)
                        <tr>
                            ...
                            <td>{{ $tools->total_tools }} {{ $tools->unity }}</td>
                            <td>
                                @empty($tools->borrow)
                                    -
                                @else
                                ...
                                <div class="row">
                                                ...
                                                <div class="py-2">
                                                    ...
                                                    @elseif ($tools->status == 1)
                                                        <a class="dropdown-item" href="#">
                                                            <form
                                                                action="{{ route('returnHK.tool', ['id' => $tools->id]) }}"
                                                                method="POST">
                                                                @csrf
                                                                <div class="row">
                                                                    <div class="col-8">
                                                                        <input class="form-control" type="number"
                                                                            name="return_qty" required>
                                                                    </div>
                                                                    <div class="col-4">
                                                    ...
                                                    <a class="dropdown-item text-danger" href="#">
                                                        <form action="{{ route('borrowHK.tool', ['id' => $tools->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            <div class="row">
                                                                <div class="col-8">
                                                                    <input class="form-control" type="number"
                                                                        name="borrow_qty" required>
                                                                </div>
                                                                <div class="col-4">
                                    ...
                                    <div class="col-auto">
                                        <button class="btn btn-info btn-sm"
                                            onclick="showModalTool({{ $tools->id }})">Edit</button>
                                    </div>
                                </div>
                            </td>
                        </tr>

                        <div class="modal fade" id="edit-tools{{ $tools->id }}" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <form action="{{ route('toolshousekeeping.update', $tools->id) }}" method="POST">
                                        @method('PATCH')
                                        @csrf
                                        <div class="modal-body">
                                            <div class="row mb-3">
                                                <div class="col-6 mb-3">
                                                    ...
                                                    <input type="text" name="name_tools" value="{{ $tools->name_tools }}"
                                                        class="form-control" required>
                                                </div>
                                                <div class=" col-6 mb-3">
                                                    <label class="form-label">Total Tools</label>
                                                    <div class="input-group">
                                                        <input type="number" min="{{ $tools->total_tools }}"
                                                            name="total_tools" value="{{ $tools->total_tools }}"
                                                            class="form-control" required>
                                                        <select name="unity" id="" class="form-control">
                                                            <option value="Tube"
                                                                {{ $tools->unity == 'Tube' ? 'selected' : '' }}>Tube</option>
                                                            <option value="Pcs"
                                                                {{ $tools->unity == 'Pcs' ? 'selected' : '' }}>Pcs</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>

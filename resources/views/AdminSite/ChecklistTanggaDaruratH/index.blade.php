                        ...
                        <div class="d-lg-flex justify-content-between">
                            <div class="row flex-between-center gy-2 px-x1 text-light">
                                <div class="col-auto pe-0">
                                    <h6 class="mb-0 text-light">List Check list Tangga Darurat</h6>
                                </div>
                            </div>
                            ...
                            <div class="d-flex align-items-center justify-content-between justify-content-lg-end px-x1">
                                ...
                                <div class="col-auto d-flex">
                                    <a class="btn btn-falcon-default btn-sm text-600" href="{{ route('checklisttanggadarurats.create') }}"><span class="fas fa-plus fs--2 me-1"></span>Create Check list Tangga Darurat</a>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="sort" data-sort="no_checklist_tangga_darurat">Nomer Check list Tangga Darurat</th>
                                    <th class="sort">Action</th>
                                </tr>
                            </thead>
                            <tbody id="checklist_body">
                                @foreach ($checklisttanggadarurats as $key => $checklisttanggadarurat)
                                    <tr>
                                        <th scope="row">{{$key + 1 }}</th>                           
                                        <td>{{ $checklisttanggadarurat->tgl_checklist }}</td>                            
                                        <td>{{ $checklisttanggadarurat->no_checklist_tangga_darurat }}</td>
                                        <td>
                                            <div class="dropdown font-sans-serif position-static"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal" type="button" id="order-dropdown-0" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                                <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="order-dropdown-0">
                                                    <div class="py-2"><a class="dropdown-item text" href="{{ route('checklisttanggadarurats.show', $checklisttanggadarurat->no_checklist_tangga_darurat) }}">Detail Tangga Darurat Checklist</a>
                                                </div>
                                            </div>  
                                         </div>                      
                                        </td>
                        ...
                        <div class="card-body">
                            <form>
                                {{-- <div class="mb-3 mt-n2">
                                    <label class="mb-1">Tanggal Checklist Tangga Darurat</label>
                                    <select class="form-select form-select-sm" name="tgl_checklist" required id="tgl_checklist">
                                        @foreach ($checklisttanggadarurats as $checklisttanggadarurat)
                                            <option value="{{ $checklisttanggadarurat->tgl_checklist }}"> {{ $checklisttanggadarurat->tgl_checklist }}</option>
                                        @endforeach
                                    </select>
                                </div> --}}
                                <div class="mb-3 mt-n2">
                                    <label class="form-label" for="timepicker2">Tanggal Checklist Tangga Darurat</label>
...
@section('script')
    ...
    <script>
        $('document').ready(function() {
            var no_checklist_tangga_darurat = $('#no_checklist_tangga_darurat').val()
            ...
            $('#tgl_checklist').on('change', function() {
                ...
                index(no_checklist_tangga_darurat, date_from, date_to)
            })
            $('#no_checklist_tangga_darurat').on('change', function() {
                var no_checklist_tangga_darurat = $('#no_checklist_tangga_darurat').val()
                var tgl_checklist = $('#tgl_checklist').val()
                var date_from = tgl_checklist.substr(0, 10)
                var date_to = tgl_checklist.substr(14, 23)

                // console.log(tgl_checklist, no_checklist_tangga_darurat)

                index(no_checklist_tangga_darurat, date_from, date_to)
            })
        })

        function index(no_checklist_tangga_darurat, date_from, date_to) {
            $.ajax({
                url: '/admin/checklist-filter-tangga_darurat',
                type: 'GET',
                data: {
                    ...
                    data.checklists.map((item, i) => {
                        $('#checklist_body').append(`
                            <tr>
                                ...
                                <td>${item.no_checklist_tangga_darurat}</td>
                                <td>
                                    <div class="dropdown font-sans-serif position-static"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal" type="button" id="order-dropdown-0" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                        <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="order-dropdown-0">
                                            <a class="dropdown-item text" href="/admin/checklisttanggadarurats/${item.no_checklist_tangga_darurat}">Detail Tangga Darurat Checklist</a>
                                        </div>
                                    </div>                        
                                </td>
                            </tr>

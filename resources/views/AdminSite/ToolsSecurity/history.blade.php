        ...
        <div class="card-header py-2">
            <div class="row flex-between-center">
                <div class="col-auto d-flex my-2 align-items-center">
                    <a href="{{ route('tools-security.index') }}" class="btn btn-falcon-default btn-sm" style="margin-right: 15px" type="button">
                ...
                <tbody id="checklist_body">
                    @foreach ($histories as $key => $history)
                        <tr>
                            ...
                            <td>{{ $history->SecTool->name_tools }}</td>
                            <td>{{ $history->action }}</td>
                            <td>{{ $history->qty }} {{ $history->SecTool->unity }}</td>

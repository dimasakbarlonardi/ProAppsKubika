    ...
    <div class="card">
        <div class="card-header py-2">
            <div class="row flex-between-center">
                <div class="col-auto">
                    <h6 class="my-3 text-light">Create Checklist Putr</h6>
                </div>
            </div>
        </div>
        <div class="p-5">
            <form method="post" action="{{ route('checklistputrs.store') }}">
                ...
                <div class="col-6 mb-3">
                    <label class="form-label">Eng Putr</label>
                    <select class="form-control" name="id_eng_putr" required>
                        <option selected disabled>-- Pilih Enggeneering Putr --</option>
                        @foreach ($engputrs as $engputr)
                        <option value="{{ $engputr->id_eng_putr }}">{{ $engputr->nama_eng_putr }} : {{$engputr->subject}} </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="row mt-4">
                    <h6>ISI DETAIL CHECKLIST PUTR <hr></h6>

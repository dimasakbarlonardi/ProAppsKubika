                        ...
                        <div class="d-lg-flex justify-content-between">
                            <div class="row flex-between-center gy-2 px-x1 text-light">
                                <div class="col-auto pe-0">
                                    <h6 class="mb-0 text-light">List Check list Koridor</h6>
                                </div>
                            </div>
                            ...
                            <div class="d-flex align-items-center justify-content-between justify-content-lg-end px-x1">
                                ...
                                <div class="col-auto d-flex">
                                    <a class="btn btn-falcon-default btn-sm text-600" href="{{ route('checklistkoridors.create') }}"><span class="fas fa-plus fs--2 me-1"></span>Create Check list Koridor</a>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="sort" data-sort="no_checklist_koridor">Nomer Check list Koridor</th>
                                    <th class="sort">Action</th>
                                </tr>
                            </thead>
                            <tbody id="checklist_body">
                                @foreach ($checklistkoridors as $key => $checklistkoridor)
                                    <tr>
                                        <th scope="row">{{$key + 1 }}</th>                           
                                        <td>{{ $checklistkoridor->tgl_checklist }}</td>                            
                                        <td>{{ $checklistkoridor->no_checklist_koridor }}</td>
                                        <td>
                                            <div class="dropdown font-sans-serif position-static"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal" type="button" id="order-dropdown-0" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                                <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="order-dropdown-0">
                                                    <div class="py-2"><a class="dropdown-item text" href="{{ route('checklistkoridors.show', $checklistkoridor->no_checklist_koridor) }}">Detail Koridor Checklist</a>
                                                </div>
                                            </div>  
                                         </div>                      
                                        </td>
                        ...
                        <div class="card-body">
                            <form>
                                {{-- <div class="mb-3 mt-n2">
                                    <label class="mb-1">Tanggal Checklist Koridor</label>
                                    <select class="form-select form-select-sm" name="tgl_checklist" required id="tgl_checklist">
                                        @foreach ($checklistkoridors as $checklistkoridor)
                                            <option value="{{ $checklistkoridor->tgl_checklist }}"> {{ $checklistkoridor->tgl_checklist }}</option>
                                        @endforeach
                                    </select>
                                </div> --}}
                                <div class="mb-3 mt-n2">
                                    <label class="form-label" for="timepicker2">Tanggal Checklist Koridor</label>
                                ...
                                <div class="mb-3 mt-n2">
                                    <label class="mb-1">Nomer Checklist Koridor</label>
                                    <select class="form-select form-select-sm" name="no_checklist_koridor" required id="no_checklist_koridor">
                                        @foreach ($checklistkoridors as $checklistkoridor)
                                            <option value="{{ $checklistkoridor->no_checklist_koridor }}"> {{ $checklistkoridor->no_checklist_koridor }}</option>
                                        @endforeach
                                    </select>
                                </div>     
                                <div class="mb-3 mt-n2">
...
@section('script')
    ...
    <script>
        $('document').ready(function() {
            var no_checklist_koridor = $('#no_checklist_koridor').val()
            ...
            $('#tgl_checklist').on('change', function() {
                ...
                index(no_checklist_koridor, date_from, date_to)
            })
            $('#no_checklist_koridor').on('change', function() {
                var no_checklist_koridor = $('#no_checklist_koridor').val()
                var tgl_checklist = $('#tgl_checklist').val()
                var date_from = tgl_checklist.substr(0, 10)
                var date_to = tgl_checklist.substr(14, 23)

                // console.log(tgl_checklist, no_checklist_koridor)

                index(no_checklist_koridor, date_from, date_to)
            })
        })

        function index(no_checklist_koridor, date_from, date_to) {
            $.ajax({
                url: '/admin/checklist-filter-koridor',
                type: 'GET',
                data: {
                    ...
                    data.checklists.map((item, i) => {
                        $('#checklist_body').append(`
                            <tr>
                                ...
                                <td>${item.no_checklist_koridor}</td>
                                <td>
                                    <div class="dropdown font-sans-serif position-static"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal" type="button" id="order-dropdown-0" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                        <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="order-dropdown-0">
                                            <a class="dropdown-item text" href="/admin/checklistkoridors/${item.no_checklist_koridor}">Detail koridor Checklist</a>
                                        </div>
                                    </div>                        
                                </td>
                            </tr>
